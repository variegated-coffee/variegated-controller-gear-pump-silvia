...
use variegated_controller_lib::{BoilerControlTarget, GroupBrewControlTarget, MachineCommand, PidParameters, PidTerm};
use variegated_hal::{DutyCycleType, TemperatureType};
            ...
            if let First(_) = either {
                ...
                match self.status.edit_mode {
                    ...
                    UIEditMode::BoilerTemperature => {
                        let boiler_params = PidParameters {
                        ...
                        self.command_sender.send(MachineCommand::SetBoilerControlTarget(0, BoilerControlTarget::Temperature(self.status.current_boiler_temp as TemperatureType, boiler_params))).await;
                    },
                    UIEditMode::PumpFlowRate => {
                        let flow_params = PidParameters {
                            kp: PidTerm { scale: 10.0, min: None, max: None },
                            ki: PidTerm { scale: 0.01, min: Some(-50.0), max: Some(80.0) },
                            kd: PidTerm { scale: 30.0, min: Some(-10.0), max: Some(10.0) }
                        };
                        ...
                        self.command_sender.send(MachineCommand::SetGroupBrewControlTarget(0, GroupBrewControlTarget::GroupFlowRate(self.status.current_flow_rate as f32, flow_params))).await;
                    },
                    UIEditMode::PumpPressure => {
                        ...
                        self.command_sender.send(MachineCommand::SetGroupBrewControlTarget(0, GroupBrewControlTarget::Pressure(self.status.current_pressure as f32, pressure_params))).await;
                    },
                }
